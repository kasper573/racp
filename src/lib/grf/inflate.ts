...
export class Inflate {
  ...
  getCode(table: HuffmanTable) {
    ...
    if (codeSize === 0 || codeSize < codeLen || codeLen === 0)
      throw new Error(
        "Bad encoding in flate stream " + codeSize + " " + codeLen
      );
  ...
  generateHuffmanTable(
    lengths: Uint32Array | Uint8Array,
    start: number,
    end: number
  ): HuffmanTable {
    ...
    let len = 1,
      code = 0,
      skip = 2;
    for (; len <= maxLen; ++len, code <<= 1, skip <<= 1) {
  ...
  readBlock() {
    ...
    if (hdr === 0) {
      ...
      let n = bufferPos;
      for (; n < end && bytesPos < bytesLength; ++n) {
    ...
    if (hdr === 1) {
      ...
      const codeLenCodeTab = this.generateHuffmanTable(
        _codeLenCodeLengths,
        0,
        19
      );
...
const fixedLitCodeTab: HuffmanTable = [
  new Uint32Array([
...
const fixedDistCodeTab: HuffmanTable = [
  new Uint32Array([
...
type HuffmanTable = [Uint32Array, number];
